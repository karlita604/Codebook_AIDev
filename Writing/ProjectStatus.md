...
- **"Structural Drift After Agentic Adoption" (2026-08-21, `paper-structure` branch, presentation-layer only — kept off `main`)**: a research-paper-format writeup of the whole project — abstract, numbered contributions (C1–C4) and RQs (RQ1–RQ5, RQ-H1, RQ-H2), related work citing `writing/LitReview.md`'s real sources, method, results, discussion, and threats to validity. Adds one synthesis not written down anywhere else before it: a three-checkpoint table (pilot N=4/12 tests, intermediate N=15/45 tests, full corpus N=79/237 tests) showing the "no consistent direction" split's significant-coefficient sign counts at each corpus scale, computed directly from the three pinned regression CSVs rather than asserted. Figures: per-repo slope-change bars, the two-panel uniform-vs-matched permutation-null comparison, and the review-intensity scatter, reusing the same underlying pinned data as the two documents above. Published: https://claude.ai/code/artifact/395cccb7-7526-4350-a2a3-c01b6e5252df . Source: `writing/artifacts/paper-structure.html` (same untracked convention as its two companions).
